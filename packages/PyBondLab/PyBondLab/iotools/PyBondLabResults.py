"""
PyBondLabResults.py -- Strategy results container and summary-table builder.

Adapted from the statsmodels summary infrastructure; see
https://github.com/statsmodels/statsmodels for the original source.

Entry points: StrategyResults.summary(), Summary (table builder)
Internal: _getnames(), _getchars(), _getturnover(), _getdates(),
          _getratings(), _getfilters(), summary_top(), summary_params(),
          summary_info(), summary_return(), forg(), table_extend()
Dependencies: statsmodels, iotools.table.SimpleTable
"""
import time
import pandas as pd
import numpy as np
...
class StrategyResults():
    """
    Container for strategy-formation results with summary-table generation.

    Stores raw returns DataFrames and parameter metadata, computes
    Newey-West t-statistics, and renders text / HTML / LaTeX summaries.

    Parameters
    ----------
    params_input : dict
        Strategy parameters (strategy object, filters, rating, etc.).
    results : dict
        Result DataFrames from ``StrategyFormation.fit()``.
    """
    ...
    def compute_tstat_pval(self, df, nw_lag=0):
        """Compute Newey-West t-statistics and p-values for each portfolio column."""
        coln = df.shape[1]
        ...
        for i in range(coln):
            ...
            if coldata.dropna().empty:
                warnings.warn(f"Portfolio '{df.columns[i+1]}' contains only NaN values.")
                continue
    ...
    # @internal
    def test(self):
        """Compute t-statistics/p-values for all result keys (called at init)."""
    ...
    def summary(self, type = "ew"):
        """
        Build a multi-table Summary for text / HTML / LaTeX rendering.

        Parameters
        ----------
        type : str
            ``'ew'`` for equal-weighted or ``'vw'`` for value-weighted.

        Returns
        -------
        Summary
            A Summary object with ``.as_text()``, ``.as_html()``,
            ``.as_latex()`` methods.
        """
        if type == 'ew':
...
# @internal
# @called-by:Summary.add_table_2cols
def summary_top(results, title=None, gleft=None, gright=None, xname=None):
    """Build the top-of-summary two-column metadata table."""
    #change of names ?
...
# @internal
# @called-by:Summary.add_table_params
...
# @internal
# @called-by:Summary.add_table_info
...
# The following helpers are verbatim (or with minor modifications) from
# statsmodels. Credits to the statsmodels project.

# @internal
def forg(x, prec=3):
    """Format a number with adaptive precision (from statsmodels)."""
    x = np.squeeze(x)
    if prec == 3:
        # for 3 decimals
        if (x) or (abs(x) < 1e-4):
...
def lmap(*args, **kwargs):
    """list(map(...)) shorthand (from statsmodels)."""
    return list(map(*args, **kwargs))

# @internal
def lrange(*args, **kwargs):
    """list(range(...)) shorthand (from statsmodels)."""
    return list(range(*args, **kwargs))

# @internal
def lzip(*args, **kwargs):
    """list(zip(...)) shorthand (from statsmodels)."""
