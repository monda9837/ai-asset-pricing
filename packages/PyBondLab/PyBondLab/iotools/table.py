"""
table.py -- Simple ASCII / CSV / HTML / LaTeX table formatter.

Adapted from the statsmodels SimpleTable implementation by Alan Isaac.
See https://github.com/statsmodels/statsmodels for the original source.
Local copy kept to avoid tight coupling with statsmodels internals.

Entry points: SimpleTable, csv2st()
Internal: Row, Cell, pad(), get_output_format()
Dependencies: statsmodels.compat.python (lmap, lrange), csv
"""
...
def get_output_format(output_format):
    """Normalise format aliases (e.g. 'text' -> 'txt', 'ltx' -> 'latex')."""
